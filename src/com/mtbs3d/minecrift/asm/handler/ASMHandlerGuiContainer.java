...
public class ASMHandlerGuiContainer extends ASMClassHandler {
	...
	public ClassTuple getDesiredClass() {
		return new ClassTuple("net.minecraft.client.gui.inventory.GuiContainer", "big");
	}
	...
	public static class FakeShiftMethodHandler implements ASMMethodHandler {
		...
		public void patchMethod(MethodNode methodNode, ClassNode classNode, boolean obfuscated) {
			...
			insnList.add(new FieldInsnNode(Opcodes.GETSTATIC, obfuscated ? "big" : "net/minecraft/client/gui/inventory/GuiContainer", "pressShiftFake", "Z"));
	...
	public static class ColorMaskMethodHandler implements ASMMethodHandler {
		...
		@Override
		public void patchMethod(MethodNode methodNode, ClassNode classNode, boolean obfuscated) {
			AbstractInsnNode findInsn = ASMUtil.findFirstInstruction(methodNode, Opcodes.INVOKEVIRTUAL, obfuscated ? "adl" : "net/minecraft/inventory/Slot", obfuscated ? "b" : "canBeHovered", "()Z", false);
			...
			insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, obfuscated ? "bqg" : "net/minecraft/client/renderer/GlStateManager", obfuscated ? "a" : "colorMask", "(ZZZZ)V", false));
			methodNode.instructions.insert(insn, insnList);
			insn = (MethodInsnNode)ASMUtil.findFirstInstruction(methodNode, Opcodes.INVOKESPECIAL, obfuscated ? "big" : "net/minecraft/client/gui/inventory/GuiContainer", obfuscated ? "a" : "drawSlot", obfuscated ? "(Ladl;)V" : "(Lnet/minecraft/inventory/Slot;)V", false);
			insnList.clear();
			insnList.add(new InsnNode(Opcodes.ICONST_1));
			insnList.add(new InsnNode(Opcodes.ICONST_1));
			insnList.add(new InsnNode(Opcodes.ICONST_1));
			insnList.add(new InsnNode(Opcodes.ICONST_1));
			insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, obfuscated ? "bqg" : "net/minecraft/client/renderer/GlStateManager", obfuscated ? "a" : "colorMask", "(ZZZZ)V", false));
			findInsn = ASMUtil.findFirstInstruction(methodNode, Opcodes.INVOKEVIRTUAL, obfuscated ? "adl" : "net/minecraft/inventory/Slot", obfuscated ? "e" : "getHasStack", "()Z", false);
